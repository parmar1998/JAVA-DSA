...
public class Solution {
	...
	public static int Count_Nodes_greater_than_x(BinaryTreeNode<Integer> root, int x) {
		if (root == null)
			return 0;
		int count_left = Count_Nodes_greater_than_x(root.left, x);
		int count_right = Count_Nodes_greater_than_x(root.left, x);
	...
	public static boolean isNode_Present(BinaryTreeNode<Integer> root,int x) {
		...
		if(root.data==x)
			return true;
		boolean isPresent_left=isNode_Present(root.left, x);
		if(isPresent_left)
			return true;
		boolean isPresent_right=isNode_Present(root.right, x);
		return isPresent_right;
	}
	
	public static void print_nodes_without_sibling(BinaryTreeNode<Integer> root) {
		if(root==null)
			return;
		if(root.left!=null&&root.right!=null) {
